		...
		if (Precondition.checkNotEmpty(argEmailUrl)) {
			...
			if (Precondition.checkNotEmpty(emailContent)) {
				...
				if (Precondition.checkNotNull(source)) {
					...
					Element contactElement = source.getFirstElement("id",
							"lhs", true);
					if (Precondition.checkNotNull(contactElement)) {
						StartTag anchorTag = contactElement
								.getFirstStartTag(HTMLElementName.A);
						if (Precondition.checkNotNull(anchorTag)) {
							String email = anchorTag.getAttributeValue("href");
