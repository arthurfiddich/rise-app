...
<%@ page import="com.rise.common.model.Person,com.rise.common.util.controller.components.Export,com.rise.common.util.controller.components.Field,java.util.*"%>
...
<%Map<String, List<Field>> entityNameVsFieldListMap = new LinkedHashMap<String, List<Field>>();%>
		...
		var fieldarray ="";
		 ...
		 $
			.ajax({
				...
				url : "/rise/export/exportajaxList?entity="+entity+"&fieldarray="+fieldarray,
				dataType : "json",
  ...
  if(str_array.length>1){
	...
	str += "</table>";
	str+="<input type='button' class='styled-button-3' value='Export' onclick=exportValues('"+data1+"','"+str_array.length+"')>";
}else{
	str += "No Data Found";
}
...
     <div class="maincontentcontainer" style="display:block;">
   ...
   entityNameVsFieldListMap = export.getEntityNameVsFieldListMap();
	...
	while(itr.hasNext()){
		Map.Entry<String, List<Field>> me = (Map.Entry<String, List<Field>>)itr.next();
		...
		%>
		<tr><td align="left"><div class="hrefstyles"><li><a href="javascript:onclick=openDetails('<%=entity %>','<%=fieldStr%>','<%=count%>','<%=length%>')"><span id="<%=count%>"><%=entity%></span></a></li></div></td></tr>
				<% 
   ...
   <div id="subrightcontent" class="subrightcontainer" style="display:none;"></div>
 </div>
     </div></div>
	</div>
</body>
