...
public class EmailExtractor {
	public static void main(String[] args) {
		...
		UrlCollector collectUrls = new UrlCollector(htmlExtractor, pageSource);
		UserInfoCollector collectUserInfo = new UserInfoCollector(htmlExtractor);
		EmailExtractor extract = new EmailExtractor();
		String url = "http://www.albany.edu/rockefeller/directory_alpha.shtml";
		try {
	...
	public void write(List<User> argUsersList) {
		...
		try {
			CsvWriter csvWriter = new CsvWriter("./output/albany/" + "albanyList-1"
					+ ".csv");
